...
degree $2N_\theta - 1$ exactly.  Convergence benchmarks on a triaxial NFW
profile show that the density reconstruction error plateaus at:
\begin{equation}
  N_\theta = \ell_{\max} + 2, \qquad
  N_\varphi = 2\ell_{\max} + 1 \quad (\text{kept odd to avoid aliasing}).
\end{equation}
Increasing beyond these values yields no accuracy improvement for smooth
profiles, reducing the total angular point count by approximately $7\times$
relative to the previous defaults.
